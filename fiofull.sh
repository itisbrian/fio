...
RDIR="${SYS_DIR}"
JOBFILETARGET="${RDIR}/fiobench.job"
OUTPUTFILETARGET="${RDIR}/fiobench.out"

echo "" > "${JOBFILETARGET}"

JOBFILEHEADER="[global] ioengine=libaio runtime=300 iodepth=128 numjobs=1 time_based direct=1"
...
cat "${RDIR}/fiobench.out" | grep "(g=" | egrep -o '^[^:]+' >> A.txt
cat "${RDIR}/fiobench.out" | grep bw= | egrep -o '^[^,]+' >> B.txt
...
echo -e "\e[32mCheck fiobench.out in the burnin directory for full results.\e[0m"
echo -e "\e[32mCheck fiosummary.txt in the burnin directory for short summary.\e[0m"
echo " "
...
OUTPUTFILETARGET="${RDIR}/fiostress.stressout"
...
echo " "
echo " "
echo -e "\e[32mStress Finished.\e[0m"
